...
checkstatus()
 ...
 while [ $opt_checkstatus != 7 ]
  ...
  echo -e "  
   1. Status of UFW\n
   2. Start UFW Services\n
   3. Stop UFW Services\n
   4. Restart UFW Services\n
   5. Flush iptables (**Use Carefully_it will remove all the rules from iptables**)\n
...
#############Check Whether the iptables installed or not############ 
 ...
 case $opt_main in
  1) echo -e "******************************"
    dpkg -l|grep iptables && dpkg -l|grep ufw 
     echo -e "******************************" ;;
  2) checkstatus ;;
  3) buildfirewall ;;
  4) exit 0 ;;
